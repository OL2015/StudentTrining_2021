...
RADIUS = 62 # in pixels
# todo read X, Y from result.ini file
...
fiber_mask = np.zeros (..., dtype = np.bool)
cm.inpaint_circular_masks(..., value = True, inside=True)
...
# add noise to fibers
noise = np.random.normal(...).astype(np.uint8)
img[fiber_mask] += ...
plt.imshow(img, cmap='gray')
plt.show()